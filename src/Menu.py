...
class Menu:
    ...
    def do_1(self):
        """Task 1 : Analyze models features inside a PED"""

        # Input management
        inputs = ask_input(task=1)
    ...
    def do_2(self):
        """Task 2 : Compare PED features between ensembles"""

        # Input management
        inputs = ask_input(task=2)
    ...
    def run(self):
        ...
        while user_input != 3:
            if repeat:
                self.generate_menu(reduced=True)
            user_input = input("Select what do you want to do: ")
    ...
    def generate_menu(self, reduced=False):
        """
