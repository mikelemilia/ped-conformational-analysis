...
def extract_names(folder, task=1):
    """
    Function to extract the PED ID (format PEDxxxxx) of the files present in the folder passed as input and return
    a list of unique ID founds. In the second task, only PED with cardinality greater than 1 can be returned.
    :param folder: folder path where to find PED IDs
    :param task: number of current task (useful to check cardinality of PED)
    ...
    if task == 2:
        ids, counts = unique(ped_names, return_counts=True)
        ids = [x for i, x in enumerate(ids) if counts[i] > 1]
    else:
        ids = unique(ped_names)
